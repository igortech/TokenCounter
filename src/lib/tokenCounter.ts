...
export async function countTokens(text: string, providerId: string, modelId: string): Promise<{ count: number }> {
  if (!text) return { count: 0 };

  if (providerId === 'openai') {
    if (modelId.includes('gpt-4o') || modelId.includes('o1') || modelId.includes('o3') || modelId.includes('gpt-5') || modelId.includes('gpt-4.5')) {
      return { count: o200k.encode(text).length };
    }
    return { count: cl100k.encode(text).length };
  }
